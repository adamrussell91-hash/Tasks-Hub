...
const KINDS = new Set<DumpKind>(['task', 'communication', 'note']);
const MAX_ITEMS = 25;
...
export function parseClareProposalJudgment(text: string, digest: ClareDumpDigest): ClareProposalJudgment {
  ...
  for (const row of rows) {
    ...
    items.push({
      ...
      domain: DOMAINS.has(domainRaw) ? (domainRaw as TaskDomain) : (digest.preferred_domain as TaskDomain),
      ...
      question: typeof bodyRow.question === 'string' && bodyRow.question.trim() ? cleanText(bodyRow.question, 240) : null
    });
  }
