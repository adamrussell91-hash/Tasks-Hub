...
- Network **Look with judgment** calls Haiku (`claude-haiku-4-5`) with a compact digest. Production needs `ANTHROPIC_API_KEY` on `artasks-hub`. Local `npm run dev` uses a stub judge when that key is missing so the button still raises flags. A Netlify scheduled function (`/api/intuitive-scan`) runs hourly and no-ops unless the Australia/Sydney hour is 6, 8, 10, 1, 3, or 6. Flags only — it never rewrites tasks.
