...
- Clare **brain dumps** call Haiku (`claude-haiku-4-5`, `CLARE_PROPOSAL_MODEL`) via `POST /api/clare` `{ action: "dump" }` to write confirm-card titles, frameworks, minutes, and reasoning. Production needs `ANTHROPIC_API_KEY` on `artasks-hub`. Without the key, local `npm run dev` falls back to heuristic proposals (regex titles + rule-based framework/minutes). Clare can call tools mid-dump: `check_clock` (hub timezone day/time) and `set_timezone` (persist from chat, stored in Blobs `meta/hub_prefs`). Default timezone is Australia/Sydney — she should look it up rather than invent dates.
