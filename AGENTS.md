...
- Clare **brain dumps** call Haiku (`claude-haiku-4-5`, `CLARE_PROPOSAL_MODEL`) via `POST /api/clare` `{ action: "dump" }` with optional `recent_thread`. She is meant to converse like Claude: questions/corrections are valid turns, not "bring a dump." Tools: `check_clock`, `set_timezone`, `read_protocol`, `update_protocol` (live manual in Blobs `agent_protocols/clare`, seeded from `config/clare-protocol.md` / `src/domain/defaults/agent-protocols.ts`). Timezone prefs: `meta/hub_prefs`. Confirm-before-write still applies to task cards. Production needs `ANTHROPIC_API_KEY` on `artasks-hub`; without it, local `npm run dev` falls back to heuristic proposals.
